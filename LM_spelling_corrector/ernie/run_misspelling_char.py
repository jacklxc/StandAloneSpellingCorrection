...
from finetune.misspelling import evaluate, predict, calculate_f1, predict_and_save
from finetune.misspelling import create_char_model as create_model
            ...
            try:
                ...
                if nccl2_trainer_id == 0 and steps % args.save_steps == 0:
                    save_path = os.path.join(args.checkpoints,
                                             "step_" + str(steps))
                    latest_path = os.path.join(args.checkpoints,
                                             "latest") # Always save the current copy and cover with the latest copy
...
def predict_wrapper(reader, exe, test_prog, test_pyreader, graph_vars,
    ...
    for test_f, save_f in zip(test_sets, save_dirs):
        ...
        predict_and_save(exe, test_prog, test_pyreader, graph_vars, save_path, reader, dev_count=1, max_len=args.max_seq_len)
