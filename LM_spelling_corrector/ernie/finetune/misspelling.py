...
from model.ernie import ErnieModel, charEncoder
...
ModelConfigs = {
    ...
    'task_type_vocab_size': 1,
    'sent_type_vocab_size': 1,
    "initializer_range": 0.02,
}

def create_char_model(args, pyreader_name, ernie_config, is_prediction=False):
    ...
    labels = fluid.layers.data(name='6', shape=[-1, args.max_seq_len, 1], dtype='int64')
    seq_lens = fluid.layers.data(name='7', shape=[-1], dtype='int64')
    char_src_ids = fluid.layers.data(name='8', shape=[-1,  ModelConfigs['max_word_len'], 1], dtype='int64')
    char_input_mask = fluid.layers.data(name='9', shape=[-1, ModelConfigs['max_word_len'], 1], dtype='float32')
    word_lens = fluid.layers.data(name='10', shape=[-1], dtype='int64')
    char_sent_ids = fluid.layers.data(name='11', shape=[-1, ModelConfigs['max_word_len'], 1], dtype='int64')
    char_pos_ids = fluid.layers.data(name='12', shape=[-1, ModelConfigs['max_word_len'], 1], dtype='int64')
    char_task_ids = fluid.layers.data(name='13', shape=[-1, ModelConfigs['max_word_len'], 1], dtype='int64')
    
    pyreader = fluid.io.DataLoader.from_generator(feed_list=[
        src_ids, sent_ids, pos_ids, task_ids, input_mask, labels, seq_lens,
        char_src_ids, char_input_mask, word_lens, char_sent_ids, char_pos_ids, char_task_ids
        ], 
        capacity=128,
        iterable=False)

    char_encoder = charEncoder(
        src_ids=char_src_ids,
        position_ids=char_pos_ids,
        sentence_ids=char_sent_ids,
        task_ids=char_task_ids,
        input_mask=char_input_mask,
        config=ModelConfigs,
        use_fp16=args.use_fp16)

    char_out = char_encoder.get_sequence_output()
    char_out = fluid.layers.reshape(x=char_out, shape=[-1, args.max_seq_len, ModelConfigs['max_word_len'], ModelConfigs['hidden_size']])
    combined_out = char_out[:,:,0,:] # Take the first token's embedding as the word's char embedding
    
    combined_out = fluid.layers.dropout(
        x=combined_out, dropout_prob=0.1, dropout_implementation="upscale_in_train")
    ...
    logits = fluid.layers.fc(
        ...
        param_attr=fluid.ParamAttr(
            name="cls_combined_seq_label_out_w",
            initializer=fluid.initializer.TruncatedNormal(scale=0.02)),
        bias_attr=fluid.ParamAttr(
            name="cl_combineds_seq_label_out_b",
...
def create_model(args, pyreader_name, ernie_config, is_prediction=False):
    ...
    ernie_out = ernie.get_sequence_output()
    ...
    char_out = char_encoder.get_sequence_output()
    char_out = fluid.layers.reshape(x=char_out, shape=[-1, args.max_seq_len, ModelConfigs['max_word_len'], ModelConfigs['hidden_size']])
    char_rep = char_out[:,:,0,:] # Take the first token's embedding as the word's char embedding

    combined_out = fluid.layers.concat(input=[ernie_out,char_rep],axis=-1)
...
def chunk_predict(np_inputs, np_probs, np_lens, dev_count=1, max_len=256):
    ...
    while True:
        try:
            inputs, probs, np_lens = exe.run(program=test_program,
                                        fetch_list=fetch_list)
            r = chunk_predict(inputs, probs, np_lens, dev_count, max_len)
            res += r
        except fluid.core.EOFException:
            test_pyreader.reset()
            break
